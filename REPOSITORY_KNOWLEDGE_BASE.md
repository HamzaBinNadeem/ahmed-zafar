...
- Meal history integration update: generated daily or weekly meal plans can be explicitly saved to Supabase `user_meal_history`, and Meal History can reopen saved daily recipes or weekly plans by hydrating `mealPlanStore`. Evidence: `zentra-1.0-main/app/meal-plan/index.tsx`, `zentra-1.0-main/app/history/meals.tsx`, `zentra-1.0-main/lib/mealPlanStore.ts`, `zentra-1.0-main/supabase/migrations/20251205220918_add_history_tables.sql`.
- Weekly meal generator integration update: the mobile API client now normalizes and validates weekly plans as complete `day1` through `day7` data, the generator screen rejects incomplete weekly output, the weekly screen renders days in stable order, and the FastAPI weekly prompt explicitly requests all seven days. Evidence: `zentra-1.0-main/lib/mealGeneratorApi.ts`, `zentra-1.0-main/app/meal-plan/index.tsx`, `zentra-1.0-main/app/meal-plan/weekly.tsx`, `backend/mealgenerator/apps/api/app/api/v1/meal_planning.py`, `backend/mealgenerator/apps/api/app/core/meal_agent.py`.
- Weekly meal UI update: weekly meal plans now render as a horizontal day-card carousel with per-day calories/protein/carbs/fat summaries, a weekly save button, and a Create Shopping List action. Shopping-list generation now clears stale cached lists on new meal-plan generation and sends a simplified food/portion payload to the FastAPI shopping endpoint. Evidence: `zentra-1.0-main/app/meal-plan/weekly.tsx`, `zentra-1.0-main/lib/mealPlanStore.ts`, `zentra-1.0-main/lib/mealGeneratorApi.ts`, `zentra-1.0-main/app/meal-plan/shopping-list.tsx`.
...
- Workout logs integration update: the workout log screen now persists per-day exercise sets to Supabase `user_logs_history`, reloads logs when the selected date changes, and shows an empty state when a day has no logs. Workout history now derives summary metrics, exercise history, and calendar intensity from saved logs instead of mock data. Evidence: `zentra-1.0-main/app/workout/index.tsx`, `zentra-1.0-main/app/workout/history.tsx`, `zentra-1.0-main/app/history/logs.tsx`, `zentra-1.0-main/supabase/migrations/20251205220918_add_history_tables.sql`.
- User metrics integration update: body metrics completion is now defined by `onboarding_completed` plus saved height and weight, signup/login routes incomplete users to `/body-metrics`, splash no longer forces logged-in users through onboarding, BMI is calculated in shared code and persisted to `user_profiles.bmi`, and profile edits recalculate BMI automatically. Evidence: `zentra-1.0-main/lib/bodyMetrics.ts`, `zentra-1.0-main/app/index.tsx`, `zentra-1.0-main/app/auth/index.tsx`, `zentra-1.0-main/app/body-metrics/weight.tsx`, `zentra-1.0-main/app/(tabs)/profile.tsx`, `zentra-1.0-main/supabase/migrations/20260429000000_add_bmi_to_user_profiles.sql`.
- Profile picture integration update: the Profile tab can pick an image from the device gallery, upload it to a public Supabase Storage `avatars` bucket, save the URL in `user_profiles.avatar_url`, and render the uploaded avatar. Evidence: `zentra-1.0-main/app/(tabs)/profile.tsx`, `zentra-1.0-main/package.json`, `zentra-1.0-main/app.json`, `zentra-1.0-main/supabase/migrations/20260429001000_create_avatar_storage_bucket.sql`.
